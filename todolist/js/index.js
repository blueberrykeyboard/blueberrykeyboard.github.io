...
window.onload = () => {
    ...
    const date = new Date();
    dateSection.innerHTML = `${date.getFullYear()}년 ${date.getMonth() + 1}월 ${date.getDate()}일 ${day[date.getDay()]}요일`;
};
    ...
    printData.forEach((item) => {
        ...
        todoItem.innerHTML = `
           <input type="checkbox" onchange="onUpdate(${item['id']})" ${item['isDone'] ? 'checked' : '   '}  />
                        ...
                        <div class="date">${date.getFullYear()}. ${date.getMonth() + 1}. ${date.getDate()} ${(date.getHours() - 12 >= 0 ? '오후' : '오전') +
            ' ' +
            String(date.getHours()).padStart(2, '0') +
            ':' +
            String(date.getMinutes()).padStart(2, '0') +
            ':' +
            String(date.getSeconds()).padStart(2, '0')}</div><button name="${item['id']}" class="button" onclick="todoDel(this)">삭제</button>
            `;
...
button.onclick = () => {
    ...
    const inputArea = document.querySelector('.Editor > input');
    const data = {
        id: idIndex++,
        isDone: false,
...
const getFilterData = (search) => {
    ...
    return mockData.filter((data) => data['content'].toUpperCase().includes(search.toUpperCase()));
    //filter의 결과를 리턴 한다.
};
